...
class ConfusionMatrix:
    ...
    def plot(self, normalize=True, save_dir="", names=()):
        """Plots confusion matrix using seaborn, optional normalization; can save plot to specified directory.
           Now generates both normalized and raw matrices.
        """
        import seaborn as sn

        # Normalized matrix (default)
        array_norm = self.matrix / ((self.matrix.sum(0).reshape(1, -1) + 1e-9) if normalize else 1)
        array_norm[array_norm < 0.005] = np.nan  # don't annotate (would appear as 0.00)

        # Raw matrix (counts)
        array_raw = self.matrix.copy()
        array_raw[array_raw < 0.005] = np.nan  # optionally hide very small values
        ...
        fig.savefig(Path(save_dir) / "confusion_matrix_raw.png", dpi=250)
        plt.close(fig)
